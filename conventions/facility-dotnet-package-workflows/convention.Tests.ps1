...
Describe 'facility-dotnet-package-workflows convention' {
	...
	It 'uses NuGet trusted publishing in the publish workflow' {
		$content = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'files' 'ci.yml') -Raw
		$content | Should -Match 'id-token: write'
		$content | Should -Match 'uses: NuGet/login@v1'
		$content | Should -Match 'user: ejball'
		$content | Should -Match 'NUGET_API_KEY: \$\{\{ steps\.nuget-login\.outputs\.NUGET_API_KEY \}\}'
		$content | Should -Not -Match 'secrets\.NUGET_API_KEY'
	}

	It 'marks published workflows as generated' {
		$header = '# DO NOT EDIT: generated by FacilityApi/Facility/conventions/facility-dotnet-package-workflows'
		foreach ($workflowName in $script:workflowNames) {
			$firstLine = Get-Content -LiteralPath (Join-Path $PSScriptRoot 'files' $workflowName) -TotalCount 1
			$firstLine | Should -Be $header
		}
	}
}
